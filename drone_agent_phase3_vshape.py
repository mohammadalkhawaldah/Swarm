"""
Phase 3 V-shape formation MAVSDK agent.

Each script instance controls a single SITL vehicle using MAVSDK. All drones
fly toward a common mission waypoint, but followers offset themselves to
create an approximate V formation:

    connect → wait for readiness → optional delay → arm → takeoff
    → compute V-waypoint → fly → loiter → land → disarm
"""
...
BASE_LOGGER = logging.getLogger("drone_agent_phase3_vshape")
...
async def connect_and_prepare(config: MissionConfig, logger: DroneLoggerAdapter) -> tuple[System, HomePosition]:
    ...
    Returns:
        Connected System instance and home position data.
    """
...
async def run_mission(config: MissionConfig) -> None:
    """Execute the V-formation mission flow."""
...
def compute_v_formation_offset(drone_id: int) -> Tuple[float, float]:
    ...
    Forward axis points from home toward target (positive toward target).
    Lateral axis positive to the right of that direction.
    """

    spacing = 30.0  # meters between ranks/wings
    mapping = {
        1: (0.0, 0.0),
        2: (-spacing, -spacing),
...
def apply_offset_to_target(
    ...
    """
    Convert formation offsets into a latitude/longitude adjustment.

    Args:
        home_lat/home_lon: Home position coordinates.
        target_lat/target_lon: Nominal target coordinates.
        forward_offset_m: Displacement along the home→target direction.
        lateral_offset_m: Displacement to the right (+) or left (-) of that direction.

    Returns:
        (assigned_lat, assigned_lon) after applying the offsets.
    """
...
def parse_args(argv: Optional[list[str]] = None) -> MissionConfig:
    ...
    parser = argparse.ArgumentParser(description="Phase 3 V-shape drone agent using MAVSDK.")
...
#
# Drone 1 (leader):
# python drone_agent_phase3_vshape.py --drone-id 1 --mavsdk-address 127.0.0.1:50040 --target-lat <LAT> --target-lon <LON> --target-alt 30
#
# Drone 2:
# python drone_agent_phase3_vshape.py --drone-id 2 --mavsdk-address 127.0.0.1:50041 --target-lat <LAT> --target-lon <LON> --target-alt 30
#
# Drone 3:
# python drone_agent_phase3_vshape.py --drone-id 3 --mavsdk-address 127.0.0.1:50042 --target-lat <LAT> --target-lon <LON> --target-alt 30
#
# Drone 4:
# python drone_agent_phase3_vshape.py --drone-id 4 --mavsdk-address 127.0.0.1:50043 --target-lat <LAT> --target-lon <LON> --target-alt 30
#
# Drone 5:
# python drone_agent_phase3_vshape.py --drone-id 5 --mavsdk-address 127.0.0.1:50044 --target-lat <LAT> --target-lon <LON> --target-alt 30
